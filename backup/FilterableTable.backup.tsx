...
export default function FilterableTable({ questions }: { questions: QuestionMetaLight[] }) {
  ...
  const questionTypes = useMemo(() => [...new Set(questions.map(q => q.type).filter(Boolean))].sort(), [questions]);
  ...
  useEffect(() => {
    setPage(1);
  }, [grade, source, questionType, numberMin, numberMax, examType, difficultyMin, difficultyMax, knowledge, tag, qidInput, viewMode]);
  ...
  const clearAll = () => {
    setGrade(''); setSource(''); setQuestionType(''); setNumberMin(''); setNumberMax('');
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.filterBar}>
        ...
        <label className={styles.filterLabel}>
          题型
          <select className={styles.filterSelect} value={questionType} onChange={e => setQuestionType(e.target.value)}>
            <option value="">全部</option>
