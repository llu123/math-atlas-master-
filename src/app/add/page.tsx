...
const doSave = async (
  ...
  try {
    ...
    let message =
      `✅ 成功入库 ${succeeded.length} 道题`;

    if (skipped.length > 0) {
      message +=
        `，跳过 ${skipped.length} 道（文件已存在）`;
    }
     ...
     * 只有真正写入至少一道题时才返回首页。
     *
     * window.location.assign('/') 会重新请求服务端首页，
     * 从而获取刚写入的最新 Markdown 题目列表。
     */
